...
import {
  approveGroupUpdateRequest,
  rejectGroupUpdateRequest,
  type AdminGroupRequestActionState,
} from "@/app/actions/admin-group-requests";

type AdminGroupUpdateRequestDecisionPanelProps = {
  requestId: number;
  status: string;
...
export function AdminGroupUpdateRequestDecisionPanel({
  requestId,
  status,
  existingAdminNote,
}: AdminGroupUpdateRequestDecisionPanelProps) {
    ...
    <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      <div className="space-y-3">
        ...
        <h2 className="text-2xl font-black tracking-tight text-slate-950">
          Approve or reject the group update
        </h2>
        <p className="max-w-3xl text-sm leading-7 text-slate-600">
          Approval applies the currently saved changes to the existing group. Requested removals are handled by deactivating those student IDs, preserving the data history.
        </p>
      </div>
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-2">
          <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5">
            ...
            <p className="mt-2 text-sm leading-7 text-emerald-900">
              Approving applies the saved rename, additions and removals to the group.
            </p>
            ...
            <form action={approveAction} className="mt-5 space-y-4">
              ...
              >
                {approvePending ? "Approving..." : "Approve and apply changes"}
              </button>
            </form>
          </div>
          ...
          <div className="rounded-3xl border border-rose-200 bg-rose-50 p-5">
            ...
            <p className="mt-2 text-sm leading-7 text-rose-900">
              Optionally update the admin note while rejecting the group update request.
            </p>
            ...
            <form
              ...
              onSubmit={(event) => {
                const confirmed = window.confirm(
                  "Reject this group update request? The request will remain stored as rejected.",
                );
            ...
            >
              ...
              <div className="space-y-2">
                <label htmlFor="groupUpdateRejectionAdminNote" className="block text-sm font-bold text-rose-900">
                  Admin note
                </label>
                <textarea
                  id="groupUpdateRejectionAdminNote"
                  name="adminNote"
